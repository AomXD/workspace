  ...
  public MemberForm selectFind(String name, String result) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        member.setQuestion(rs.getString(5));
        member.setResult(rs.getString(6));
...
//添加信息
  public void insertMember(MemberForm form) {
    try {
      ...
      ps.setString(4, form.getQuestion());
      ps.setString(5, form.getResult());
  ...
  public void updateMember(MemberForm form) {
    try {
      ps = connection.prepareStatement("update tb_member set name=?,password=?,reallyName=?,question=?,result=?,phonenumber? where id=?");
      ps.setString(1, form.getName());
      ps.setString(2, form.getPassword());
      ps.setString(3, form.getReallyName());
      ps.setString(4, form.getQuestion());
      ps.setString(5, form.getResult());
      ps.setString(6, form.getPhoneNumber());
      ps.setString(7, form.getId().toString());
      ps.executeUpdate();
      ps.close();
    }
    catch (SQLException ex) {
